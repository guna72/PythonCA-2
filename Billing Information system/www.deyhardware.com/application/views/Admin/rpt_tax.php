    ...
    <table class="table table-striped table-hover text-center" style="border: 1px solid #dff;box-shadow: 2px 2px 14px #888"  id="table">
      ...
      <tbody>
        ...
        foreach ($InvoiceDetails as $row) {?>
          ...
          <td colspan='2' class='text-center'><?php echo $row->con_invoice_no;?></td>
          <td colspan='2' class='text-center'><?php echo $row->date;?></td>
          <td colspan='2' class='text-center'><?php echo $row->cgst;?>%</td>
         
           <?php $totl_value =round($row->grand_total_not_gst - ($row->grand_total_not_gst * $row->discount_total )/100) ;?>
          <td colspan='2' class='text-center'><?php echo round(($totl_value * $row->cgst )/100);?></td>
          <td colspan='2' class='text-center'><?php echo $row->sgst;?>%</td>
          <td colspan='2' class='text-center'><?php echo round(($totl_value * $row->sgst )/100);?></td>
		  <td colspan='2' class='text-center'><?php $total = $total + (round(($totl_value * $row->cgst )/100) + round(($totl_value * $row->sgst )/100)); 
		  echo (round(($totl_value * $row->cgst )/100) + round(($totl_value * $row->sgst )/100)); ?></td>

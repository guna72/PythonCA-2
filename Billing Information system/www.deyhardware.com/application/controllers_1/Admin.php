...
public function edit_product_details()
{
  ...
  if($discount==NULL)
   ...
   $data=['product_name'=>$product_name,'mrp'=>$mrp,'discount'=>$discount,'count'=>$count,'purches_amount'=>$purches_amount];
   ...
   if($this->form_validation->run('invoice_rules'))
   {
    ...
    { 
      ...
      while($x<=strlen($t))
      {
       ...
       $data[$k]=['ph_no'=>$ph_no,'customer_name'=>$row->customer_name,'product_name'=>$product_name,'sac'=>$prod_details->sac,'mrp'=>$mrp,'discount'=>$prod_details->discount,'qnty'=>$qnty,'cgst'=>$cgst,'sgst'=>$sgst,'igst'=>$igst,'date'=>$date,'grand_total_not_gst'=>$total,'current'=>1,'no_invoice_day'=>$max_no_invoice_day,'date_diffr'=>$diff->format('%R%a')];
       $x++;
     }

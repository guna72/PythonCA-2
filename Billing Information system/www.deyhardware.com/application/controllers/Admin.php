...
public function create_product(){
 $this->load->helper('form');
 $ProductypeDetails = $this->adminmodel->getProductType();
 $this->load->view('Admin/create_product_entries_view.php',['ProductypeDetails'=>$ProductypeDetails]);
}
// Add Product entries ------------------------------
public function add_product()
{
  if($this->input->post()){
   ...
   if($this->form_validation->run('add_product_rules')){
    ...
    $discount=0.00;//$this->input->post('discount');
  ...
  if($val==-1)
  {
      if($this->adminmodel->add_product($product_name,$sac,$mrp,$discount,$count,$purches_amount,$product_type))
    {
...
public function present_stock_product()
{
    ...
    if($this->input->post())
  {
   ...
   $this->load->helper('form');
   $ProductypeDetails = $this->adminmodel->getProductType();
   $prod_details=$this->adminmodel->get_products($product_name,$product_type);
        
   $this->load->view('Admin/create_stock_view.php',['prod_details'=>$prod_details,'ProductypeDetails'=>$ProductypeDetails,
...
public function edit_product_details()
{
  ...
  {
   $sac=$this->input->post('product_code');
   $sac_old =$this->input->post('product_code_old');
   $product_name=$this->input->post('product_name');
   $mrp=$this->input->post('mrp');
   $purches_amount=$this->input->post('purches_amount');
  ...
  if($discount==NULL)
   ...
   $data=['product_name'=>$product_name,'sac'=>$sac,'mrp'=>$mrp,'discount'=>$discount,'count'=>$count,'purches_amount'=>$purches_amount];
    
   if($this->adminmodel->update_product_details($data,$sac_old))
   {
...
public function create_invoice()
{
 $this->load->helper('form');
 $ProductypeDetails = $this->adminmodel->getProductType();
 $this->load->view('Admin/create_invoice_view.php',['ProductypeDetails'=>$ProductypeDetails]);
}
// Storing Invoice--------------------------
   ...
   if($this->form_validation->run('invoice_rules'))
   {
    ...
    { 
      ...
      while($x<=strlen($t))
      {
		...
		$discount =$this->input->post('total_discounts');
		echo 'discount'.$discount;
		//die;
        $product_prices=$this->input->post('product_price_sss'.$t[$k]);
        if($cgst==NULL)
          $cgst=0;
        if($sgst==NULL)
          $sgst=0;
          ...
          if($product_prices!=null)
            {
              ...
              {
              $mrp=$prod_details->mrp;
            }
			// a - 200 - 2
			// b - 300 -1 
			// c - 400 -2 
			// discount - 10 %  (1500 - 150 = 1350)
        $total_without_qnty= $mrp;  //-($mrp*($prod_details->discount/100));
        $total = $total_without_qnty * $qnty;
       
       $data[$k]=['ph_no'=>$ph_no,'customer_name'=>$row->customer_name,'product_name'=>$prod_details ->product_name,'sac'=>$prod_details->sac,'mrp'=>$mrp,'discount'=>$prod_details->discount,'qnty'=>$qnty,'cgst'=>$cgst,'sgst'=>$sgst,'igst'=>$igst,'date'=>$date,'grand_total_not_gst'=>$total,'current'=>1,'no_invoice_day'=>$max_no_invoice_day,'date_diffr'=>$diff->format('%R%a')];
       $x++;
     }
        $this->session->set_flashdata(['data_main'=>$data,'ph_no'=>$ph_no,'customer_name'=>$row->customer_name,'t'=>$t,'discount_total'=>$discount]);
        return redirect('Admin/create_invoice');
  }
  else
  {
 ...
 public function end_submit()
 {
    if($this->input->post())
    {
       ...
       while($x<$p)
       {
        ...
        $data[$x]=['ph_no'=>$ph_no,'customer_name'=>$customer_name,'product_name'=>$product_name,'sac'=>$sac,'mrp'=>$mrp,'discount'=>$discount,'qunty'=>$qnty,'cgst'=>$cgst,'sgst'=>$sgst,'igst'=>$igst,'date'=>$date,'grand_total_not_gst'=>round($total),'payment'=>$g_payment,'no_invoice_day'=>$no_invoice_day,'date_diffr'=>$date_diffr,'active'=>$active,'discount_total'=>$total_discount,'payment_mode'=>$payment_mode];
	  ...
	  $invoice_id = $this->adminmodel->add_invoice($data,$active,$x);
      if($invoice_id>0)
      {
           if($active==0)
           {
              $data_send=['Date'=>$date,'no_invoice_day'=>$no_invoice_day,'ph_no'=>$ph_no,'payment'=>$g_payment,'total'=>$all_total];
              if(!$this->adminmodel->partial_payment($data_send,$invoice_id))
              {
                $this->session->set_flashdata('feedback1','Storing Failed Try Again!');
                return redirect('Admin/create_invoice');
              }
           }
        $this->session->set_flashdata('feedback2','Successfully Payment Completed');
 ...
 public function back()
 {
   if($this->input->post())
   {
    // echo "FROM ADMIN ".$this->input->post('val');
     $val=strtolower($this->input->post('val'));
       //echo $val;
     $prd_type_id =$this->input->post('prd_type_id');
       //echo '-type-'.$prd_type_id;
      $res=$this->adminmodel->get_hints($val,$prd_type_id);
      // print_r($res);
      $c='<ul style="list-style-type:none" class="hints_list">';
      for($i=0;$i<count($res);$i++)
      {
...
// +++++++++++++++++++++++End Of Show Hints++++++++++++++++++++++
 ...
 {
 $this->load->helper('form');
 $ProductDetails = $this->adminmodel->get_productsss("0");
 ...
 $this->load->helper('form');
      $this->load->library('form_validation');
        if($this->form_validation->run('add_parches_product_rules')){
            $data=$this->input->post();
            $flg=$this->adminmodel->check_invoice($data);
        if($flg==0){
         if($this->adminmodel->add_product_parches($data))
         {
                ...
                $flashdata=$this->session->set_flashdata('feedback2',$data['invoice_no'].'  -invoice no alredy exsist in the system');
                return redirect('Admin/parches_product');
         } 
 ...
 $VendorDetails = $this->adminmodel->get_vendor_details('');
 $this->load->view('Admin/parches_view',['ProductDetails'=>$ProductDetails,'VendorDetails'=>$VendorDetails,'product_type'=>0]);
 } 
 ...
 //return redirect('Admin/parches_product');
    ...
    public function tax_report()
    {
        ...
        if($this->input->post()){
        $frm_date= date("Y-m-d", strtotime($this->input->post('from_date')));
        $to_date= date("Y-m-d", strtotime($this->input->post('to_date')));
        
        $frm_date_view= date("d-m-Y", strtotime($this->input->post('from_date')));
        $to_date_view= date("d-m-Y", strtotime($this->input->post('to_date')));
        
        
        $data_inv=$this->adminmodel->get_invoice_date($frm_date,$to_date);
            $this->load->view('Admin/rpt_tax',['InvoiceDetails'=>$data_inv,'frm_date'=>$frm_date_view,'to_date'=>$to_date_view]);
        }else{
 ...
 $VendorDetails = $this->adminmodel->get_vendor_details('');
   $vendor_id=0;
    ...
    if($this->input->post()){
        ...
        $i=0;
            foreach ($rptDetails as $row ) 
            {
                
                $amountDetails=$this->adminmodel->getInvoiceamountss($row->invoice_no);
                $data[$i]['cheque_no']='na';
                $data[$i]['amount_pay']=0;
                $data[$i]['payment_date']=0;
                if(!empty($amountDetails)){
                foreach ($amountDetails as $row1 ) {
                   $data[$i]['amount_pay'] = $row1['payment_amount'];
                   $data[$i]['cheque_no'] = $row1['cheque_no'];
                   $data[$i]['payment_date'] = $row1['payment_date'];
                }}
                
                $data[$i]['invoice_no']=$row->invoice_no;
                $data[$i]['invoice_date']=$row->invoice_date;
                $data[$i]['total_amount']=$row->amount;
                $data[$i]['payment_status']=$row->payment_status;
                $i++;
                
            }
       $rptDetails= $data;
            $this->load->view('Admin/rpt_vendor',['VendorDetails'=>$VendorDetails,'rpt_vendor'=>$rptDetails,'vendor_id'=>$vendor_id]);
        }
    else
    {
        $this->load->view('Admin/rpt_vendor',['VendorDetails'=>$VendorDetails,'vendor_id'=>$vendor_id]);
    }
}    
public function employee_add_view() {
 ...
 public function add_employee()
 {
    ...
    public function get_product_mrp()
    {    
        if($this->input->post())
        {
        $prd_code =$this->input->post('product_code');
        $res=$this->adminmodel->getProductPrice($prd_code);
    ...
    {    
        $VendorDetails = $this->adminmodel->get_vendor_details('');
        $this->load->view('Admin/purches_payment',['VendorDetails'=>$VendorDetails]);
    }
    ...
    public function PaymentProduct()
     {    
            $this->load->helper('form');
            $this->load->library('form_validation');
            if($this->form_validation->run('purches_payment_ruls'))
            {
                $mrp=$this->input->post('product_amounts');
                $amount_pay =$this->input->post('amount_pay');
                if($amount_pay>$mrp)
                { 
                     $flashdata=$this->session->set_flashdata('feedback2','Enter correct Amount');
                        return redirect('Admin/payment_product');    
                }
                $flage=0;
                if($mrp==$amount_pay){
                    $flage=1;
                ...
                $data=$this->input->post();
                if($this->adminmodel->savePaymentProduct($data,$flage))
                  {
                    $flashdata=$this->session->set_flashdata('feedback1','Successfully done.');
                        return redirect('Admin/payment_product');    
                  } 
            }   
            else
        ...
        $res=$this->adminmodel->getInvoiceamount($invoice_no);
        echo $res;*/
        
     }    
    
    public function getInviceDetails()
    {    
       if($this->input->post())
        {
    // echo "FROM ADMIN ".$this->input->post('val');
     $val=strtolower($this->input->post('val'));
       $vendor_id=$this->input->post('vendor_id');
           //echo $vendor_id;
      $res=$this->adminmodel->get_invoice_details($val,$vendor_id);
      // print_r($res);
      $c='<ul style="list-style-type:none" class="hints_list">';
      for($i=0;$i<count($res);$i++)
      {

...
public function add_invoice($data,$active,$x)
{
  ...
  while ( $x !=-1)
  {
	  $q="UPDATE invoice SET con_invoice_no = $flag_invoice , con_chanal_no = $flag_chalan where invoice_no=$invoice_id";
...
public function set_invoice_payment($ph_no,$date,$no_invoice_day,$total_payment,$q,$current_max_invoice)
{
   ...
   if($this->db->affected_rows())
   {
	  $flg111=$this->update_manage_id(1,$cheque_no+1);
      return TRUE;
   }
   else
     return FALSE;
...
public function get_invoice_date($form,$to)
{
 ...
 $q="SELECT p.date,p.con_invoice_no,sum(p.grand_total_not_gst) as grand_total_not_gst,p.cgst,p.sgst,p.discount_total FROM invoice p WHERE STR_TO_DATE(p.date,'%d-%m-%Y') between '$form' and '$to' and con_invoice_no!=0 group by con_invoice_no";
...
public function get_employee_details($employee_name)
{
  ...
  return $q->result();*/
      ...
      public function get_profit_loss_data($form,$to)
      {
          ...
          $q="(SELECT 'S' as type,sum(b.grand_total_not_gst) as pay_amount,
		  0 as total_amount FROM invoice b where 
		  date between '$form' AND '$to' and b.active=1) 
		  UNION (SELECT 'E' as type,sum(c.payment_amount) as pay_amount ,
		  0 as total_amount FROM employee_salery_details c 
		  WHERE date_format(c.date_of_payment, '%d-%m-%Y') between '$form' AND '$to')
		  UNION (SELECT 'P' as type ,sum(payment_amount) as pay_amount ,0 as total_amount 
		  from parches_payment where date_format(payment_date, '%d-%m-%Y') between '$form' AND '$to' )";
		 // echo $q;
		 // die;
          $query=$this->db->query($q);
          return $query->result();

...
public function add_invoice($data,$active)
{
	$flag_chalan =0;
	$flag_invoice =0;
	$flg=0;
  ...
  if($this->db->affected_rows())
  {
	  $this->update_manage_id($flg,$flg_value);
    return $invoice_id;
	} 
  else { 
    $invoice_id;
...
public  function partial_payment($data_send,$invoice_id)
{
  ...
  if($this->db->affected_rows() )
  {
	  $q2="update invoice set con_chalan_no=$chalan_id where invoice_no=$invoice_id";
...
{
	$q="SELECT * FROM invoice a,partial_payment b,customer c 
	WHERE a.no_invoice_day = b.no_invoice_day and c.customer_name=a.customer_name 

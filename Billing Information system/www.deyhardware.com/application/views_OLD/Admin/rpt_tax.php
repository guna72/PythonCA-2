    ...
    <table class="table table-striped table-hover text-center" style="border: 1px solid #dff;box-shadow: 2px 2px 14px #888"  id="table">
      ...
      <tbody>
        ...
        foreach ($InvoiceDetails as $row) {?>
          ...
          <td colspan='2' class='text-center'><?php $total = $total + ($row->payment - $row->grand_total_not_gst); echo number_format($row->payment - $row->grand_total_not_gst);?></td>
        </tr>
 <?php 
$p++;} }
?>
 ...
 <tr>
          ...
          <td colspan='2' class='text-center'></td>
          <td colspan='2' class='text-center'></td>
          <td colspan='2' class='text-center'><?php echo number_format($total)." /-"?></td>
        </tr>
          <?php } ?>
</tbody>
</table>

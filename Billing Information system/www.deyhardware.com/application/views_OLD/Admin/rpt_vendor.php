    ...
    <table class="table table-striped table-hover text-center" style="border: 1px solid #dff;box-shadow: 2px 2px 14px #888"  id="table">
      <thead>
        <tr >
          ...
          <th colspan="2" class="text-center">Billing Amunt</th>
      ...
      <tbody>
        ...
        foreach ($rpt_vendor as $row) {?>
          ...
          <td colspan='2' class='text-center'><?php echo date('d-m-Y', strtotime($row->date));?></td>
          <td colspan='2' class='text-center'><?php echo $row->product_id;?></td>
          <td colspan='2' class='text-center'><?php echo $row->amount;?></td>
          <td colspan='2' class='text-center'><?php echo $row->pay_amount;?></td>
          <td colspan='2' class='text-center'><?php $total =($row->amount - $row->pay_amount); echo number_format($total);?></td>
        </tr>
 <?php 
$p++;} }
?>
